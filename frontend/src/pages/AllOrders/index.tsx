...
export function AllOrders() {
  ...
  const loadOrders = async (reset = false) => {
    try {
      ...
      const result = await orderService.getAll(limit, currentOffset);
  ...
  return (
    <div className="orders-page">
      <div className="orders-header">
        <h1>Pedidos</h1>
        <Link to="/admin/orders/new" className="add-order-btn">
          ➕ Novo Pedido
        </Link>
      </div>
                ...
                <div>
                  ...
                  {order.customer && (
                    <span style={{ marginLeft: '1rem', color: '#666', fontSize: '0.9rem' }}>
                      Cliente: {order.customer.nickname}
                      {order.customer.document_number && ` (${order.customer.document_number})`}
                    </span>
                  )}
                </div>
